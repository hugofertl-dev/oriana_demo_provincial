...
LINT_CMD="{{LINT_CMD}}"            # ej: flutter analyze | npx eslint . | ruff check .
TEST_CMD="{{TEST_CMD}}"            # ej: flutter test    | npm test     | pytest
BUILD_CMD="{{BUILD_CMD}}"          # ej: flutter build web | npm run build
SRC_DIRS="{{SRC_DIRS}}"            # ej: "lib test" — dirs cuyo cambio exige verificación
...
[ -n "$GITDIR" ] && \
  git status --porcelain -- $SRC_DIRS 2>/dev/null | sha1sum | cut -d' ' -f1 > "$GITDIR/harness_verify_ok" || true
echo ""
